...
describe("pagina de login", () => {
  it("Registrar Usuario", () => {
    ...
    cy.get(selectorsList.newsletterContains).contains(
      "Sign up for our newsletter!",
    ); // Verifica o texto do checkbox de newsletter
    ...
    cy.get(selectorsList.adressTwoField).type(
      userData.userSucess.userStreetTwo,
    ); // Preenche o campo de endereço 2
    cy.get(selectorsList.countrySelector).select(
      userData.userSucess.userCountry,
    ); // Seleciona o país
    ...
    cy.get(selectorsList.postalCodeField).type(
      userData.userSucess.userPostalCode,
    ); // Preenche o campo de CEP
    cy.get(selectorsList.mobileNumberField).type(
      userData.userSucess.userNumber,
    ); // Preenche o campo de número de telefone
  ...
  it("Login Usuario", () => {
    cy.visitAndClickLogin(); // Visita a página inicial e clica no botão de login
    cy.login(
      userData.userSucess.useremalogin,
      userData.userSucess.userpassword,
    ); // Faz login com email e senha válidos
